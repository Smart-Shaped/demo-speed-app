...
public class TrendingReviewsUpdater extends SpeedUpdater {
	...
	public Dataset<Row> updateSpeed(Dataset<Row> df) throws SpeedUpdaterException {
		...
		df = df.withWatermark("timestamp", WATERMARK)
				.groupBy(functions.window(functions.col("timestamp"), WINDOW_DURATION, SLIDE_DURATION),
						functions.col("asin"))
				.agg(functions.avg("rating").alias("meanrating"));

		df = df.where(functions.col("meanrating").$greater$eq(4.5));

		df = df.withColumn("windowstart", functions.col("window.start"))
				.withColumn("windowend", functions.col("window.end")).drop("window");
